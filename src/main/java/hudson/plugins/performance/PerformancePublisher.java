    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher,
        ...
        for (PerformanceReportParser parser : parsers) {
            ...
            for (PerformanceReport r : parsedReports) {
                ...
                //get prev build
                PerformanceBuildAction performanceBuildAction = build.getPreviousBuild().getAction(PerformanceBuildAction.class);
                if (performanceBuildAction == null) {
                    PerformanceReport prevReport = performanceBuildAction.getPerformanceReportMap().getPerformanceReport(
                            r.getReportFileName());
                    for (String key : r.getUriReportMap().keySet()) {
                        long prev = prevReport.getUriReportMap().get(key).getMax();
                        long curr = r.getUriReportMap().get(key).getMax();
                        if (prev / curr * 100 > errorUnstableThreshold) result = Result.UNSTABLE;
                        if (prev / curr * 100 > errorFailedThreshold) result = Result.FAILURE;
                    }
                }
